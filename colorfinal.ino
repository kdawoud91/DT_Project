...
const int lamppin = 2; // the pin that the LED is attached to
const int fanpin=7;
...
//////////////////////////////////////  
  ...
  digitalWrite(lamppin, HIGH); // will turn off the lamp
  digitalWrite(fanpin, HIGH); // will turn on the fan
...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (incomingByte == 'H') {
      ...
      delay(300);
      digitalWrite(fanpin,LOW); // will stop the fan
      pixels.clear();
    ...
    if (incomingByte == 'K') {
      ...
      digitalWrite(fanpin,LOW); // will stop the fan
      pixels.clear();
      pixels.fill(pixels.Color(0, 0, color));
      pixels.show();    
